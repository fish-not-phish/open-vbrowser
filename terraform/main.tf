...
module "ecs_tasks" {
  ...
  ecs_task_role_arn      = module.infrastructure.ecs_task_role_arn
  ecs_execution_role_arn = module.infrastructure.ecs_task_execution_role_arn

  task_cpu                 = var.task_cpu
  task_memory              = var.task_memory
  task_resource_overrides  = var.task_resource_overrides
...
# ── Append Terraform outputs to the .env file after apply ─────────────────────
locals {
  env_path     = "${path.module}/../docker/.env"
