...
THORIUM_VERSION="128.0.6613.189"
THORIUM_URL="https://github.com/Alex313031/thorium/releases/download/M128.0.6613.189/thorium-browser_128.0.6613.189_AVX.deb"

# Download the Thorium browser deb package and install it
wget -q -O /tmp/thorium-browser.deb "${THORIUM_URL}"
apt install -y /tmp/thorium-browser.deb
rm -f /tmp/thorium-browser.deb

# Create a symbolic link for easy launching of Thorium
ln -sf /usr/lib/floorp/floorp /usr/bin/startfloorp

# Create a desktop entry for Thorium
cat << EOF > /tmp/thorium-browser.desktop
[Desktop Entry]
Version=1.0
...
mv /tmp/thorium-browser.desktop /usr/share/applications/thorium-browser.desktop
