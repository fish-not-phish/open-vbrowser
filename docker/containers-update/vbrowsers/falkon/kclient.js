...
// ----------------------------------------------------------------
// Updated: Screenshot functionality for canvas inside noVNC_container
// ----------------------------------------------------------------
// Function to request the screenshot from the iframe
function screenshot() {
  const iframe = document.querySelector('iframe.vnc');
  if (!iframe) {
    alert('VNC iframe not found.');
    return;
  }
  // Request the iframe to capture the screenshot
  iframe.contentWindow.postMessage({ type: 'captureScreenshot' }, '*');
}

// Listen for the screenshot data coming from the iframe
